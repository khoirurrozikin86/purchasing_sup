...
class PurchaseRequestController extends Controller
{
    ...
    public function AllpurchaserequestWaiting()
    {
        return view('purchase_request.all_purchaserequestwaiting');
    }
    ...
    public function GetpurchaserequestCountPR(){
    ...
    public function GetpurchaserequestCount(){
        $requestCount = PurchaseRequestDetail::count();
  ...
  public function GetpurchaserequestCountPending(){
     
       $requestCount = PurchaseRequest::select('purchase_request_no')
       ->whereHas('detailrequest', function ($query) {
           $query->whereNull('status')        // Status detail yang null
                 ->orWhere('status', '');    // Status detail yang kosong (string kosong)
       })
       ->groupBy('purchase_request_no')      // Kelompokkan berdasarkan 'purchase_request_no'
       ->havingRaw('COUNT(*) > 0')           // Pastikan ada detail dengan status kosong atau null
       ->count();
    ...
    public function Getpurchaserequest(Request $request)
{
    if ($request->ajax()) {
        ...
        $query = PurchaseRequest::with(['detailrequest.item.unit', 'detailrequest.item.category', 'cbd', 'user'])->orderBy('created_at', 'desc');
        ...
        return Datatables::of($data)
            ...
            ->addColumn('item_code', function ($row) {
                // Menampilkan item_code dalam <ul><li></li></ul> untuk setiap detail request (seperti color)
                $details = '<ul>';
                foreach ($row->detailrequest as $detail) {
                  
                    if (!empty($detail->item->item_code)) {
                        $details .= '<li>' . $detail->item->item_code . '</li>';
                    }
                }
                $details .= '</ul>';
                
                // Jika tidak ada item_code yang ditemukan, kembalikan spasi kosong
            ...
            ->addColumn('category', function ($row) {
                // Menampilkan category name dalam <ul><li></li></ul> untuk setiap detail request (seperti item_name)
                $details = '<ul>';
                foreach ($row->detailrequest as $detail) {
                    $categoryName = $detail->item->category->name ?? '-';
                    $details .= '<li>' . $categoryName . '</li>';
                }
                $details .= '</ul>';
                return $details;
            })
          ->addColumn('item_name', function ($row) {
    // Menampilkan item_name dalam <ul><li></li></ul> dengan potongan 25 karakter
    $details = '<ul>';
    foreach ($row->detailrequest as $detail) {
        // Jika item_name null, gunakan '-' sebagai default
        $itemName = $detail->item->item_name ?? '-';
        
        // Potong nama item menjadi 25 karakter jika panjangnya lebih dari 25
    ...
    return $details;
            ...
            ->addColumn('user_name', function ($row) {
                return $row->user->name ?? '-';  // Menambahkan user_name dari relasi user
            })
            ->rawColumns(['action','item_code', 'category', 'item_name', 'color', 'size','unit', 'total', 'remark', 'status'])
            ->make(true);
    }
}

public function Getpurchaserequestwaiting(Request $request)
{
    if ($request->ajax()) {
        // Start with the base query
        $query = PurchaseRequest::with(['detailrequest.item.unit', 'detailrequest.item.category', 'cbd', 'user'])
                                ->orderBy('created_at', 'desc');
        
        // Filter Purchase Requests with PurchaseRequestDetails that have status NULL or empty
        $query->whereHas('detailrequest', function ($query) {
    ...
    public function Getpurchaserequest00(Request $request)
{
    if ($request->ajax()) {
        // Start with the base query
 ...
 private function generatePurchaseRequestNo()
{
    ...
    $prefix = '/PR/PCH/SUP/' . $currentMonth . '/' . $currentYear;
    ...
    $newNumber = 1; // Default to 1 if no previous request exists

    if ($latestRequest) {
        // Ambil tahun dari purchase_request_no terakhir
        $lastYear = substr($latestRequest->purchase_request_no, -4); // Mengambil tahun dari bagian akhir nomor PR

        // Jika tahun pada request terakhir sama dengan tahun sekarang
        if ($lastYear == $currentYear) {
            // Ambil nomor urut dari purchase_request_no yang terakhir dan ekstrak 6 digit pertama
            $lastNumber = (int) substr($latestRequest->purchase_request_no, 0, 6);
    ...
    // Gabungkan nomor urut dengan prefix
    return $newNumber . $prefix;
}

      private function generatePurchaseRequestNo1()
{
    $currentMonth = Carbon::now()->format('m');
    $currentYear = Carbon::now()->format('Y');
    $prefix = '/PR/PCH/SUP/' . $currentMonth . '/' . $currentYear;
    ...
    public function Storepurchaserequest(Request $request)
    {
        ...
        return redirect()->route('all.purchaserequest')->with('message', 'Purchase Request berhasil disimpan.');
    }
...
if ($idx) {
    ...
    // Definisikan urutan ukuran yang diinginkan
    $sizeOrder = ['XXS', 'S', 'M', 'L', 'XL', 'XXL'];

    // Ambil data size, color, dan qty dari detail cbd
    foreach ($cbd->details as $detail) {
        // Masukkan size dan color ke dalam array
        $sizes[$detail->size] = $detail->size;
        $colors[$detail->color] = $detail->color;
        $qtyData[$detail->size][$detail->color] = $detail->qty;
    }

    // Urutkan sizes sesuai dengan urutan kustom
    usort($sizes, function ($a, $b) use ($sizeOrder) {
        $aIndex = array_search($a, $sizeOrder);
        $bIndex = array_search($b, $sizeOrder);

        // Jika ukuran tidak ditemukan dalam urutan yang didefinisikan, beri nilai yang lebih tinggi
        if ($aIndex === false) $aIndex = count($sizeOrder); // Tempatkan di akhir
        if ($bIndex === false) $bIndex = count($sizeOrder); // Tempatkan di akhir

        return $aIndex - $bIndex;
    });

    // Urutkan colors secara alfabet
    sort($colors);
